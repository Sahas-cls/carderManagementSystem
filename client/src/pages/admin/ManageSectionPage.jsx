...
const ManageSectionPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { sectionName: form.sectionName.trim() };
    ...
    try {
      if (!editingId) {
        ...
        await editSection(editingId, payload);
        Swal.fire({
          title: "Success!",
  ...
  const handleEdit = (row) => {
    setForm({ sectionName: row.sectionName || "" });
    setEditingId(row.id);
    setShowForm(true);
  };
  ...
  const handleDelete = async (row) => {
    ...
    try {
      ...
      Swal.fire({
        title: "Deleted!",
        text: `'${row.sectionName}' has been deleted.`,
        icon: "success",
        timer: 2000,
        showConfirmButton: false,
      });
  ...
  return (
    <div className="p-4">
      ...
      <div className="mt-4 rounded-md">
        <Card title="Manage Sections" variant="navy">
          <div className="p-4 overflow-auto">
            {loading ? (
              <div className="text-center text-sm text-slate-400 py-10">Loading sections...</div>
            ) : sections.length === 0 ? (
