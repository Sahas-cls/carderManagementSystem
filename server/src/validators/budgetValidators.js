...
/** Validates the optional ?factoryId= query param used to scope GET /budgets. */
...
/**
 * Validates the create/update body. The client sends { budget: { factoryId, moCount, tmoCount } } -
...
function validateBudgetBody(req, res, next) {
  try {
    const source = req.body?.budget ?? req.body ?? {};
    ...
    const moCount = parseCount(source.moCount, "MO");
    const tmoCount = parseCount(source.tmoCount, "TMO");

    req.body = { factoryId, moCount, tmoCount };
    next();
  } catch (err) {
    next(err);
  }
...
/** Validates the body of PATCH /budgets/:id/status - accepts { status } or { budget: { status } }. */
...
module.exports = { validateIdParam, validateFactoryQuery, validateBudgetBody, validateStatusBody };
